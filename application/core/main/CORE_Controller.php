...
abstract class CORE_Controller extends CI_Controller {
    ...
    public function Display($viewName='',$isAjaxContentFirst=true){
		...
		if(empty($viewName)){	
		    ...
		    if(!empty($this->view_controler_dir)){	
		      $dirname=app_rtrim($this->view_controler_dir,'/');
		    }
	...
	protected function AddIntoPageList(){
		if(ENVIRONMENT!="production"){
			...
			$mpa=new Mpage_list();
			$class=app_str_replace("_", "-", $this->router->class);
			$method=app_str_replace("_", "-",$this->router->method);
			$dir=app_str_replace("_", "-",$this->router->directory);
			$dir=app_rtrim($this->router->directory,'/');
			$panel=get_panel_by_dir();			
			$controller_title=app_str_replace("_", " ",$this->router->class);
			...
			if(!in_array($class."/*", $ppagelist) && !in_array($class."/".$method, $ppagelist)){
				if(!$mpa->IsExists("controller", $class,array("directory"=>$dir,"method"=>$method))){
					$title=app_str_replace("_", " ",$this->viewData['_title']);
