	...
	static function CompletePayment($ticket_payment_obj,$customer_name,$card_or_payment_email,$total_amount,$transaction_id,$transaction_time,$approval_code,$resull_msg='',$country='',$payment_method='P')
    {
        ...
        if ($mpayment->Save()) {
            ...
            if ($upayment->Update()) {
                ...
                Mdebug_log::AddOtherLog(
                    "Payment update failed :" . $ticket_payment_obj->ticket_id,
                    Mdebug_log::STATUS_FAILED,
                    Mdebug_log::ENTRY_TYPE_ERROR,
                    GetMsgForAPI() . "\n" . print_r($ticket_payment_obj, true)
                );
            }
        }
        return false;
