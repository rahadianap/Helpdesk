...
class Mcustom_field extends APP_Model{
	...
	public function GetPropertyRawOptions($property,$isWithSelect=false){
		...
		switch ($property) {
	      case "type":        
	         $returnObj=array("T"=>"Textbox","N"=>"Numeric","F"=>"Float Number","A"=>"Date","E"=>"Time","M"=>"DateTime","O"=>"Switch" , "R"=>"Radio","D"=>"Dropdown","L"=>"Text/Instruction","U"=>"URL Input");
	         break;
	     case "is_required":        
	...
	static function getCustomFieldsByCategory($cat_id,&$fieldsNeedTObeCheck=[],$added_category=[]){
		...
		foreach (self::$customfieldAllCategories as $fld){
			...
			if(in_array($cat_id,$cats) || ($cat_id!='R' && in_array('0',$cats))){
