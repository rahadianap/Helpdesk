...
class Mticket extends APP_Model
{
    ...
    public static function getFileUrl($file,$ticket_user_id,$objobject=null){
        $utype = GetCurrentUserType();
        $hasHtaccess=dirname($file->path)."/.htaccess";
        if(Mapp_setting::GetSettingsValue('use_direct_file','N')=='Y'){
            ...
            if(!file_exists($hasHtaccess)){
                file_put_contents(dirname($file->path).DIRECTORY_SEPARATOR.".htaccess", "<IfModule authz_core_module>\n\tRequire all denied\n</IfModule>\n<IfModule !authz_core_module>\n\tDeny from all\n</IfModule>");
            }
            if($objobject instanceof  Mticket){                 return $utype == "AD" ? (base_url("admin/ticket/ticket-img/{$file->hash}/{$ticket_user_id}/{$objobject->id}/{$file->name}")):(base_url("ticket/ticket-img/{$file->hash}/{$ticket_user_id}/{$objobject->id}/{$file->name}"));
            }elseif($objobject instanceof  Mticket_reply){
                return $utype == "AD" ? (base_url("admin/ticket/ticket-replied-file/{$file->hash}/{$ticket_user_id}/{$objobject->ticket_id}/{$objobject->reply_id}/{$file->name}")) : (base_url("ticket/ticket-replied-file/{$file->hash}/{$ticket_user_id}/{$objobject->ticket_id}/{$objobject->reply_id}/{$file->name}"));
            }
        }
    ...
    static function SendTicketOpenEmailByObj($ticket_obj)
    {
        if ($ticket_obj instanceof self) {
            ...
            if ($emailobj->SendEmailTemplates($kword, $tuserobj->email, "", $params,true)) {
                return true;
            }
        } else {
            return false;
    ...
    WHERE role='$role_id' and  app_user.status='A'
