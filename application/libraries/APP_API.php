...
class APP_API{
    ...
    final static function AddLoadedAPI($fullClass,$loaded_by=""){
        if (class_exists($fullClass)) {
            self::$loaded_by[$fullClass]=$loaded_by;
            self::$_loaded_api[$fullClass] = new $fullClass();
            self::$_loaded_api[$fullClass]->initial_loading();
            return true;
        }
    }
    private static function loaded_plugin($api_name,$api_fullname,$path)
    {
	...
	private static function load_plugin($api_name,$api_fullname,$path){
		if(file_exists($path)){
			include_once $path;
			if(class_exists($api_fullname)){
				self::$_loaded_api[$api_name]=new $api_fullname();
