...
class Imap {
	...
	public function imap_connect($config = [])
	{
        ...
        elseif ($config['encrypto'] != NULL && isset($config['encrypto']) && $config['encrypto'] == 'stls')
        {
            $enc = '/imap/tls';
        }
	...
	public function set_unseen_message($id = 0, $unseen = TRUE)
	{
		...
		$flags = '';
		$flags .= (strlen(app_trim($header->Answered)) > 0 ? '\\Answered ' : '');
		$flags .= (strlen(app_trim($header->Flagged)) > 0 ? '\\Flagged ' : '');
		$flags .= (strlen(app_trim($header->Deleted)) > 0 ? '\\Deleted ' : '');
		$flags .= (strlen(app_trim($header->Draft)) > 0 ? '\\Draft ' : '');
		$flags .= (($unseen == FALSE) ? '\\Seen ' : ' ');

		imap_clearflag_full($this->stream, $id, '\\Seen', ST_UID);

		return imap_setflag_full($this->stream, $id,app_trim($flags), ST_UID);
	}
	...
	protected function format_message($id = 0, $with_body = TRUE, $embed_images = TRUE)
	{
		...
		$email = [
			...
			'unread'   => strlen(app_trim($header->Unseen)) > 0,
			'answered' => strlen(app_trim($header->Answered)) > 0,
			'flagged'  => strlen(app_trim($header->Flagged)) > 0,
			'deleted'  => strlen(app_trim($header->Deleted)) > 0,
			'size'     => $header->Size,
		];
		if (isset($header->cc))
		{
