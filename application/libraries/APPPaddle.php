...
class APPPaddle extends AppPaymentBase {
    ...
    public function process_payment($payment_id,$payment_obj,$controller)
    {
        ...
        $data['prices'] = array($payment_obj->payment_currency . ':' . ($payment_obj->amount));           $data['return_url'] = site_url("site/action/paddle-response/S/{$payment_obj->ticket_id}/{$payment_obj->reply_id}/{$payment_obj->id}");
    ...
    function VerifySignature($postvalue){
        $public_key_string =app_trim($this->public_key);
