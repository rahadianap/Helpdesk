...
class Compiler
{
    ...
    protected function compileWith($withCondition)
    {
        ...
        if ($withCondition) {
            ...
            if ($withConfig !== null) {
                $without = [];                 $list = $this->coerceList($withConfig);
            ...
            if ($withoutConfig !== null) {
                $without = [];                 $list = $this->coerceList($withoutConfig);
        ...
        switch ($arg[0]) {
            ...
            case Type::T_EXPRESSION:
                if ($arg[1] === '/') {
                    ...
                    $arg[5] = $arg[6] = false;                     $arg = $this->expToString($arg);
                }
                break;
    ...
    private function evaluateArguments(array $args, $reduce = true)
    {
        ...
        foreach ($args as $arg) {
            if (!empty($arg[0])) {
                ...
                } elseif ($val[0] !== Type::T_NULL) {                     $positionalArgs[] = $this->maybeReduce($reduce, $val);
                }
