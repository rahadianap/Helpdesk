...
class LightOpenID
{
    ...
    function __set($name, $value)
    {
        switch ($name) {
        case 'identity':
            if (strlen($value =app_trim((String) $value))) {
    ...
    protected function request_curl($url, $method='GET', $params=array(), $update_claimed_id)
    {
        ...
        if($method == 'HEAD' || $method == 'GET') {
            ...
            foreach(explode("\n", $header_response) as $header) {
                $pos = strpos($header,':');
                if ($pos !== false) {
                    $name = strtolower(app_trim(substr($header, 0, $pos)));
                    $headers[$name] =app_trim(substr($header, $pos+1));
                }
            }
        ...
        if (!$url) throw new ErrorException('No identity supplied.');
            ...
            if ($yadis) {
                ...
                if (isset($headers['x-xrds-location'])) {
                    $url = $this->build_url(parse_url($url), parse_url(app_trim($headers['x-xrds-location'])));
                    $next = true;
                }
                ...
                if (isset($headers['content-type']) && $this->is_allowed_type($headers['content-type'])) {
                    ...
                    foreach($m[1] as $content) {
                        ...
                        if(preg_match('#<Type>\s*'.$ns.'(server|signon)\s*</Type>#s', $content, $type)) {
                            ...
                            $server = $server[1];
                            if (isset($delegate[2])) $this->identity =app_trim($delegate[2]);
                            $this->version = 2;
                ...
                if (isset($this->headers['x-xrds-location'])) {
                    $url = $this->build_url(parse_url($url), parse_url(app_trim($this->headers['x-xrds-location'])));
                    continue;
                }
        ...
        if (empty($hint) || $this->getItem('openid_ns_' . $hint) != $namespace) {
            ...
            foreach ($this->data as $key => $val) {
                if (strncmp($key, $prefix, $length) === 0 && $val === $namespace) {
                    $result =app_trim(substr($key, $length));
                    break;
                }
            }
        } else {
